...
**Generate-then-verify on full prose.** The most common production approach: generate the full document with retrieval-augmented context, then run a verification pass over the output. Its real strength is simplicity — one generation pass, one verification pass — and it avoids the skeleton-to-prose leakage problem entirely, because there's no intermediate representation to leak from. The reason I chose the skeleton approach over this is that verification is substantially harder on free-form prose: claims are embedded in rhetoric, hedging, and compound sentences, and the claim-extraction step introduces its own errors before verification even starts. But this is a genuine trade-off, not a clear win — the skeleton approach trades the extraction problem for the leakage problem. If prose rendering introduces new claims at a high rate despite constraints, generate-then-verify with better extraction may have been the right call. This is what Week 2 validation is designed to test.

**Chunked generation with per-section verification.** Generate and verify one section at a time, feeding verified sections as context for the next. This directly attacks context dilution — my root cause #1 — without the skeleton abstraction: each section gets fresh model attention on its source material, and verification happens on shorter, more manageable prose chunks. It also preserves natural writing flow within sections. The skeleton approach's advantage is that verification operates on clean structured inputs rather than prose — but that advantage only holds if the prose rendering step is well-behaved. The chunked approach trades structured verifiability for a simpler pipeline with no leakage risk. I chose the skeleton because I believe the verification-quality advantage outweighs the leakage risk, but if Week 2 shows high leakage rates, chunked generation becomes the fallback.


**Constrained decoding / grounding-aware generation.** Force the model to only produce text it can trace back to source material. In theory, the cleanest fix — prevent hallucination at the point of writing rather than catching it afterward. It also requires direct access to model weights — it works by intervening in the token probability distribution before each word is chosen, which API-based models like Claude or GPT-4 don't expose. But the deeper problem is that it breaks down for long-form documents because not every sentence *should* be traceable to a source. Transitions, framing, and analysis are a normal part of writing — [research shows](https://arxiv.org/abs/2404.07060) that 25–50% of sentences in well-formed long-form answers are inherently ungrounded, even when the content is correct. Constrained decoding can't tell the difference between a factual claim that needs a citation and a sentence like "this matters because..." that doesn't. Applying it broadly ruins the writing; applying it selectively requires solving a classification problem that's just as hard as the original verification problem.

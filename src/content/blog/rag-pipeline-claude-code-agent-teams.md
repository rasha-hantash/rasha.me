...
The problem is that building each variant takes time. Different chunking strategies, different retrieval methods, different reranking backends — each one touches multiple files across parsing, storage, retrieval, and generation. That's where the workflow comes in: I used <a href="https://code.claude.com/docs/en/agent-teams" target="_blank">Claude Code agent teams</a> to build the baseline RAG system — the full four-PR stack with infrastructure, ingestion, retrieval, and frontend. Once the baseline exists, each variant is just one prompt to Claude Code, stacked as a separate PR with <a href="https://graphite.dev" target="_blank">Graphite</a> — so you're comparing clean diffs instead of untangling one massive branch. Run your evals against each branch, compare the numbers, merge the winner.
